...
void configureADC(ADC_t *adc) {
    ...
    GPIOA->MODER |= GPIO_MODER_MODE0 | GPIO_MODER_MODE1 | GPIO_MODER_MODE2 | GPIO_MODER_MODE4
                  | GPIO_MODER_MODE5 | GPIO_MODER_MODE6;  // Analog mode for pins PA0, PA1, PA2, PA4, PA5, PA6
    ...
    ADC1->CHSELR |= ADC_CHSELR_CHSEL0 | ADC_CHSELR_CHSEL1 | ADC_CHSELR_CHSEL2 | ADC_CHSELR_CHSEL4
                  | ADC_CHSELR_CHSEL5 | ADC_CHSELR_CHSEL6;  // Select channels to be converted
...
void readADC(ADC_t* adc) {
    ...
    while (i < ADC_CHANNELS) {
        ...
        ADC1->SMPR = ADC_SMPR_SMP1_0 | ADC_SMPR_SMP1_1 | ADC_SMPR_SMP1_2; // slowest sampling time
        ...
        adc->values[i] = ADC1->DR; // store the ADC conversion result
        i++;
    }
}
...
void configADC(void) {
    ...
    ADC1->CFGR2 &= ~ADC_CFGR2_CKMODE; // select HSI16 as clock source
    ADC1->SMPR = 7; // slowest sampling time
    ADC1->CHSELR = (ADC_CHSELR_CHSEL0 | ADC_CHSELR_CHSEL1 | ADC_CHSELR_CHSEL2 | ADC_CHSELR_CHSEL4 | ADC_CHSELR_CHSEL5 | ADC_CHSELR_CHSEL6); // select channels 0, 1, 2, 4, 5 and 6
    ADC1->IER = 0; // disable interrupts
    ADC1->CR |= ADC_CR_ADEN; // enable ADC
    while ((ADC1->ISR & ADC_ISR_ADRDY) == 0); // wait for ADRDY flag to be set
}
...
void movingAverage(ADC_t *adc){
	for (int adcIdx = 0; adcIdx < ADC_CHANNELS; adcIdx++) {
		...
		adc->adcMA[adcIdx] = adc->adcSum[adcIdx] / ADC_WINDOW_SIZE;
	}
}
