...
//	i2c1_scanner(addrs);
	...
	m24c64_read_N(BASE_ADDR, &att, ATT_VALUE_ADDR, 1);
	...
	m24c64_read_N(BASE_ADDR, rcv_buff, POUT_IS_CALIBRATED_ADDR, 1);
	is_calibrated = rcv_buff[0] == AD8363_IS_CALIBRATED ? true : false;
	if (!is_calibrated) {
		send_buff[0] = AD8363_ADC_MIN >> 8;
		send_buff[1] = AD8363_ADC_MIN & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, POUT_ADC_MIN_ADDR_0, 2);
		send_buff[0] = AD8363_ADC_MAX >> 8;
		send_buff[1] = AD8363_ADC_MAX & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, POUT_ADC_MAX_ADDR_0, 2);
	}

	m24c64_read_N(BASE_ADDR, rcv_buff, PIN_IS_CALIBRATED_ADDR, 1);
	is_calibrated = rcv_buff[0] == MAX4003_IS_CALIBRATED ? true : false;
	if (!is_calibrated) {
		send_buff[0] = MAX4003_ADC_MIN >> 8;
		send_buff[1] = MAX4003_ADC_MIN & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, PIN_ADC_MIN_ADDR_0, 2);
		send_buff[0] = MAX4003_ADC_MAX >> 8;
		send_buff[1] = MAX4003_ADC_MAX & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, PIN_ADC_MAX_ADDR_0, 2);
	}

	m24c64_read_N(BASE_ADDR, rcv_buff, VSWR_IS_CALIBRATED_ADDR, 1);
	is_calibrated = rcv_buff[0] == MAX4003_IS_CALIBRATED ? true : false;
	if (!is_calibrated) {
		send_buff[0] = MAX4003_ADC_MIN >> 8;
		send_buff[1] = MAX4003_ADC_MIN & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, VSWR_ADC_MIN_ADDR_0, 2);
		send_buff[0] = MAX4003_ADC_MAX >> 8;
		send_buff[1] = MAX4003_ADC_MAX & 0xff;
		m24c64_write_N(BASE_ADDR, send_buff, VSWR_ADC_MAX_ADDR_0, 2);
	}
	...
	m24c64_page_read(6, 6, data);
	uint8_t data_write[2];
	data_write[0] = 0x77;
	data_write[1] = 0x88;

	m24c64_write_N(6, data_write, 4, 2);

	HAL_Delay(10);
	m24c64_read_N(6, data_read, 4, 2);
