...
uint8_t m24c64_1byte_read(uint8_t addr) {
	uint8_t data;
	i2c1_byteTransmit(CHIP_ADDR<<1 , &addr,1);
	data = i2c1_byteReceive(CHIP_ADDR <<1 | 1,1);
	return data;
}
...
void m24c64_1byte_write(uint8_t addr, uint8_t data) {
	...
	if (stored_data != data)
		i2c1_byteTransmit((char ) CHIP_ADDR<<1 , buff,2);
}
...
void m24c64_write(uint8_t addr, uint16_t data) {
	...
	if (stored_data != data)
		i2c1_byteTransmit((char ) CHIP_ADDR << 1, buff,3);
}
...
uint16_t m24c64_read(uint8_t address) {
	...
	i2c1_byteTransmit((char )CHIP_ADDR << 1, &address,1);
